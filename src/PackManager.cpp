#define LOG_CONTEXT "BedrockAPI::PackManager"

#include "PackManager.h"

#include <Zenova/JsonHelper.h>
#include <Zenova/Log.h>
#include <Minecraft/ResourceHeaders.h>
...
namespace Zenova {
	...
	bool addToPack(Packs& packs, const std::string& path) {
		...
		if (!modDocument.IsNull()) {
			...
			if (headerObj.IsObject()) {
				...
				Zenova_Info("{}: {}", packLocation, packs.back().uuid);
				return true;
			}
		}
	...
	const Packs& PackManager::getPacks(PackType type) {
		switch (type) {
			...
			default:
				Zenova_Error("Unknown PackType {}", fmt::underlying(type));
				return getResourcePacks();
		}
	}
}